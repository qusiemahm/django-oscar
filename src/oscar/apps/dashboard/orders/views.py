...
class OrderPopupDetailView(EventHandlerMixin, DetailView):
    """
    Dashboard view to display a single order.
    ...
    model = Order
    context_object_name = "order"
    template_name = "oscar/dashboard/orders/order_popup_detail.html"
...
def change_order_status(request):
    if request.method == "POST":
        ...
        if new_status in ["Accepted", "Rejected"]:
            ...
            messages.success(request, f"Order {order.number} status changed to {new_status}.")
            return JsonResponse({"success": True, "message": f"Order status changed to {new_status}"})
        
        return JsonResponse({"success": False, "message": "Invalid status selection."})
